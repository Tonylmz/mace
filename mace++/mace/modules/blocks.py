...
class RealAgnosticResidualInteractionBlock(InteractionBlock):
    def _setup(self) -> None:
        ...
        # Square the tensor
        self.message_square = o3.TensorSquare(self.edge_attrs_irreps).irreps_out.simplify()

        self.self_message_product = o3.FullyConnectedTensorProduct(
            self.edge_attrs_irreps, self.edge_attrs_irreps, self.message_square
        )

        irreps_mid_square, instructions_square = tp_out_irreps_with_instructions(
            self.node_feats_irreps,
            self.message_square,
            self.target_irreps, 
        )

        self.conv_tp_square = o3.TensorProduct(
            self.node_feats_irreps,
            self.message_square,
            irreps_mid_square,
            instructions=instructions_square,
            shared_weights=False,
            internal_weights=False,
        )

        self.conv_tp_weights_square = nn.FullyConnectedNet(
            [input_dim] + self.radial_MLP + [self.conv_tp_square.weight_numel],
            torch.nn.functional.silu,
        )

        self.linear_square = o3.Linear(
            irreps_mid_square, self.irreps_out, internal_weights=True, shared_weights=True
        )
    ...
    def forward(
        ...
        edge_feats: torch.Tensor,
        edge_index: torch.Tensor,
        hyper_edge_attrs: torch.Tensor,
        hyper_edge_feats: torch.Tensor,
        ...
        edge_attrs_square = self.self_message_product(edge_attrs, edge_attrs)

        tp_weights_square = self.conv_tp_weights_square(edge_feats)

        mji_square = self.conv_tp_square(
            node_feats[sender], edge_attrs_square, tp_weights_square
        )  # [n_edges, irreps]
        
        message_square = scatter_sum(
            src=mji_square, index=receiver, dim=0, dim_size=num_nodes
        ) # [n_nodes, irreps]
        
        message_square = self.linear_square(message_square) / self.avg_num_neighbors
        
        message_square = self.reshape(message_square)

        tp_hyper_weights = self.conv_tp_weights(hyper_edge_feats)
        # hyper_index = torch.arange(0, node_feats.shape[0]).to(node_feats.device)
        message_hyper = self.conv_tp(
            hyper_node_feats, hyper_edge_attrs, tp_hyper_weights
        )  # [n_edges, irreps]
        
        message_hyper = self.linear(message_hyper) / self.avg_num_neighbors
        
        message_hyper = self.reshape(message_hyper)

        message = message + message_square
